  ...
  useEffect(() => {
    const handleClickMenuOutside = event => {
      if (
        ...
        !event.composedPath().includes(menuBlockRef.current)
      ) {
        setMenuOpen(false);
      }
    };

    document.body.addEventListener('click', handleClickMenuOutside);
    return () => {
      document.body.removeEventListener('click', handleClickMenuOutside);
    };
  });
        ...
        {isAuth ? (
          <>
            <Link to="/lk" className={styles.menuBlock__user}>
              ...
              <p className={styles.menuBlock__userEmail}>
                Kuropatka1233@mail.ru
              </p>
            </Link>
